...
import com.learnwithme.onlineshopping.util.FileUploadUtility;
import com.learnwithme.onlineshopping.validator.ProductValidator;
...
public class ManagementController {
	...
	public ModelAndView showManageProducts(@RequestParam(name="operation", required=false) String operation) {
		...
		if(operation != null) {
			if(operation.equals("product")) {
				...
				mv.addObject("message", "Category Added Submitted!");
			}
		}
	...
	public ModelAndView showEditProducts(@PathVariable int id) {
		...
		Product newProd = productDAO.get(id);
	...
	public String handleProductSubmission(@Valid @ModelAttribute("product") Product mProduct, BindingResult results, 
		...
		if(mProduct.getId() == 0) {
			...
			// update the product record if the id is 0
			productDAO.update(mProduct);
		}
		
		
		// Image to be uploading
		if(!mProduct.getFile().getOriginalFilename().equals("")) {
	...
	/**
	 * @return List of categories for all the request mapping
	 */
